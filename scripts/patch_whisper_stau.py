...
def apply_learnable_stau(
    ...
    """给每个 WhisperAttention 挂一个 STauOpusLearnable（融合算子），τ 和 α 可学习。
    ...
    for name, module in model.named_modules():
        ...
        opus = STauOpusLearnable(h, sigma_name=sigma_map[kind]).to(next(module.parameters()).dtype)
    ...
    print(f"[τ-opus] Attached learnable STauOpus to {n_attached} attention modules")
