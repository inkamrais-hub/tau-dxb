...
class STauOpusAttentionFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_out, grad_attn_unused):
        ...
        d_s_stable = tau * attn * (grad_attn - weighted)   # (B,H,Lq,Lk) — d_log_sigma
        d_s_stable = d_s_stable * (sigma_p / sigma_val) * alpha  # reuse same variable

        # Backward through max-stable shift: scatter_add_ in-place on d_s_stable
        sum_dy = d_s_stable.sum(dim=-1, keepdim=True)
        d_s_stable.scatter_add_(-1, argmax, -sum_dy)  # d_s_stable ← d_scores, no clone

        # Backward through scores = Q @ K^T * scale
        grad_Q = torch.matmul(d_s_stable, Kf) * scale  # (B,H,Lq,D)
        grad_K = torch.matmul(d_s_stable.transpose(-2, -1), Qf) * scale  # (B,H,Lk,D)
